...
import localWardrobeData from "@/data/local-wardrobe.json";
...
const Wardrobe = () => {
  ...
  const fetchItems = async () => {
    ...
    const allItems = [...(localWardrobeData as WardrobeItem[])];
    if (!error && data) {
      allItems.push(...(data as WardrobeItem[]));
    } else if (error) {
      toast.error("Failed to load wardrobe from cloud");
    }
    setItems(allItems);
    setLoading(false);
  };
  ...
  const addItem = async () => {
    ...
    // Local insert
    try {
      await fetch('/api/save-local-wardrobe', {
        method: 'POST',
    ...
    if (error) {
      toast.error("Failed to add item to cloud");
    } else {
      toast.success("Item added to cloud and local repository!");
      setDialogOpen(false);
